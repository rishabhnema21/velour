...
export const addToLibrary = async (req, res) => {
    try {
        ...
        const existingShelfBook = await db
            .select()
            .from(shelfBooks)
            .where(and(eq(shelfBooks.shelfId, shelf[0].id), eq(shelfBooks.userBookId, userBookId)))
            .limit(1);
    ...
    try {
        ...
        if (shelf.isSystem) {
            ...
            if (systemShelfIds.length > 0) {
                await db
                    .delete(shelfBooks)
                    .where(and(eq(shelfBooks.userBookId, userBookId), inArray(shelfBooks.shelfId, systemShelfIds)));
            }
        }
        await db
            .insert(shelfBooks)
