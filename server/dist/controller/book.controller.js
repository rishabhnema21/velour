...
export const getBooks = async (req, res) => {
    try {
        ...
        const searchCondition = or(ilike(books.title, `%${query}%`), ilike(books.description, `%${query}%`));
        ...
        const filteredResult = googleSearchResult.filter((item) => {
        ...
        const booksToInsert = filteredResult.map((item) => ({
            ...
            isbn10: item.volumeInfo.industryIdentifiers?.find((id) => id.type === "ISBN_10")?.identifier,
            isbn13: item.volumeInfo.industryIdentifiers?.find((id) => id.type === "ISBN_13")?.identifier,
        }));
